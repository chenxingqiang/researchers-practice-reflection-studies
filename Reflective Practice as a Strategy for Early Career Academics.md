...
graph LR
    ...
    C -->|Results in| E[Lack of Research Progress]
    D -->|Causes| F[Insufficient Time for Both]
    E -->|Contributes to| G[Career Development Challenges]
    F --> G
    H[Evaluation Pressures] -->|Exacerbates| D
    I[Lack of Experience] -->|Affects| D

    %% Style definitions
    classDef main fill:#f0f4f8,stroke:#2c5282,stroke-width:2px;
    ...
    linkStyle 0,1,2,3,4,5,6 stroke:#2c5282,stroke-width:1.5px;
    linkStyle 7,8 stroke:#4a5568,stroke-width:1px,stroke-dasharray: 5 5;
```
...
```mermaid
graph LR
    A[Implicit PUA in Academia] -->|Leads to| B{Manifestations}
    B -->|Includes| C[Unrealistic Expectations]
    B -->|Involves| D[Emotional Manipulation]
    B -->|Results in| E[Excessive Control]
    C -->|Causes| F[Stress and Anxiety]
    D --> F
    E --> F
    F -->|Leads to| G[Decreased Job Satisfaction]
    F -->|Contributes to| H[Reduced Academic Freedom]
    F -->|Impacts| I[Impaired Work-Life Balance]
    ...
    %% Link styles
    linkStyle default stroke:#4a5568,stroke-width:1px;
    linkStyle 0,1,2,3,4,5 stroke:#2c5282,stroke-width:1.5px;
    linkStyle 6,7,8 stroke:#4a5568,stroke-width:1px,stroke-dasharray: 5 5;
```
...
```mermaid
graph LR
    A[Reflective Practice] -->|Leads to| B[Reflection-in-action]
    A -->|Involves| C[Knowing-in-action]
    A -->|Includes| D[Reflection-on-action]
    B -->|Enables| E[Reshaping Ongoing Action]
    C -->|Involves| F[Tacit Knowledge in Action]
    D -->|Encourages| G[Retrospective Contemplation]
    E -->|Leads to| H[Improved Practice]
    F --> H
    G --> H
    ...
    %% Link styles
    linkStyle default stroke:#4a5568,stroke-width:1px;
    linkStyle 0,1,2 stroke:#2c5282,stroke-width:1.5px;
    linkStyle 3,4,5,6 stroke:#4a5568,stroke-width:1px,stroke-dasharray: 5 5;
```
...
```mermaid
graph TD
    A[Reflective Practice Framework] -->|Inquires| B[How can it be done?]
    A -->|Analyzes| C[What is accomplished?]
    A -->|Evaluates| D[Are methods/goals worthwhile?]
    A -->|Reflects on| E[Who should I be?]
    B -->|Leads to| F[Strategic Planning]
    C -->|Leads to| G[Evaluating Progress]
    D -->|Considers| H[Ethical Consideration]
    E -->|Influences| I[Identity Formation]
    F -->|Enhances| J[Improved Time Management]
    G -->|Facilitates| K[Goal Alignment]
    H -->|Guides| L[Value-Driven Work]
    I -->|Promotes| M[Increased Confidence]
    ...
    linkStyle default stroke:#4a5568,stroke-width:1px;
    linkStyle 0,1,2,3 stroke:#2c5282,stroke-width:1.5px;
    linkStyle 4,5,6,7,8,9,10,11 stroke:#4a5568,stroke-width:1px,stroke-dasharray: 5 5;
```
...
<img src="/Users/xingqiangchen/Downloads/Relationship%20between%20Level%20of%20Reflection%20and%20Satisfaction%20with%20Teaching-Research%20Integration.png" style="zoom: 33%;" />
...
```mermaid
graph LR
    A([Reflective Practice]) ==>|Influences| B(Teaching-Research<br>Integration)
    A ==>|Reduces| C(Academic Anxiety)
    A ==>|Enhances| D(Personal Autonomy)
    A ==>|Shapes| E(Disciplinary Adaptation)
    B & C & D --> F{Academic<br>Satisfaction}
    E --> G(Tailored Approaches)
    F & G ==>|Improves| H((Academic<br>Performance))
    
    classDef main fill:#f0f4f8,stroke:#2c5282,stroke-width:2px;
    classDef process fill:#e6f6ff,stroke:#2b6cb0,stroke-width:1.5px;
    classDef mediator fill:#fff5f5,stroke:#c53030,stroke-width:1.5px;
    classDef outcome fill:#f0fff4,stroke:#2f855a,stroke-width:1.5px;
    ...
    linkStyle default stroke:#4a5568,stroke-width:1px;
    linkStyle 0,1,2,3,6 stroke:#2c5282,stroke-width:1.5px;
    linkStyle 4,5 stroke:#4a5568,stroke-width:1px,stroke-dasharray: 5 5;
